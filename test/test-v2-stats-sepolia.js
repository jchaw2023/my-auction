...
async function main() {
  ...
  if (network.name !== "sepolia") {
    ...
    console.log("请使用: npx hardhat run test/test-v2-stats-sepolia.js --network sepolia");
    process.exit(1);
  }
  ...
  console.log("==========================================");
  console.log("测试 MyAuctionV2 统计信息功能");
  console.log("==========================================");
  ...
  try {
    ...
    try {
      ...
      if (version.toString() !== "2") {
        console.log("   ⚠️  警告: 合约版本不是 V2，可能尚未升级");
      }
    } catch (error) {
    ...
    const platformFee = await myAuction.platformFee();
    ...
    const auctionCount = await myAuction.getAuctionCount();
    ...
    if (auctionCount > 0n) {
      for (let i = 0; i < Math.min(Number(auctionCount), 5); i++) {
        try {
          const bidCount = await myAuction.getAuctionBidCount(i);
