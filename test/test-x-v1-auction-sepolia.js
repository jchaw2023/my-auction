...
/**
 * V2 版本测试：获取 Sepolia 测试网上的拍卖总数
 * 
 * 此脚本用于测试 MyXAuction V2 版本的拍卖查询功能
 * 可以获取合约中创建的拍卖总数，并列出所有拍卖的基本信息
 * 
 * 使用方法：
 * npx hardhat run test/test-x-v2-auction-sepolia.js --network sepolia
 */
async function main() {
  // 检查网络
  if (network.name !== "sepolia") {
    console.error("❌ 此脚本只能在 Sepolia 测试网上运行");
    console.log("请使用: npx hardhat run test/test-x-v2-auction-sepolia.js --network sepolia");
    process.exit(1);
  }
  ...
  console.log("==========================================");
  console.log("V2 版本测试：获取拍卖总数 (Sepolia 测试网)");
  console.log("==========================================");
  console.log(`网络: ${network.name}`);
  console.log(`账户: ${signerAddress}`);
  console.log(`MyXAuction 地址: ${myAuctionAddress}`);
  console.log(`MyNFT 地址: ${myNFTAddress}`);
  console.log("==========================================\n");

  // 连接合约
//   const MyXAuction = await ethers.getContractFactory("MyXAuctionV2");
  const myXAuction = await ethers.getContractAt("MyXAuctionV2", myAuctionAddress);

  // 获取拍卖总数
  console.log("📊 获取拍卖总数...\n");

  try {
    const count = await myXAuction.getAuctionCount();
    ...
    if (count > 0n) {
      ...
      for (let i = 0; i < Number(count); i++) {
        const auction = await myXAuction.getAuction(i);
        const status = auction.ended ? "已结束" :
          (BigInt(Math.floor(Date.now() / 1000)) < auction.startTime ? "未开始" : "进行中");
        console.log(`  拍卖 #${auction.tokenId.toString()}: ${status}`);
