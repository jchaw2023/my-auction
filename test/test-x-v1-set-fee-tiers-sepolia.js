...
/**
 * 测试设置 MyXAuction 的动态手续费档次
 * 
 * 使用方法：
 * npx hardhat run test/test-x-v1-set-fee-tiers-sepolia.js --network sepolia
 * 
 * 设置自定义手续费档次（示例）：
 * npx hardhat run test/test-x-v1-set-fee-tiers-sepolia.js --network sepolia
 */
async function main() {
    ...
    if (network.name !== "sepolia") {
        ...
        console.log("请使用: npx hardhat run test/test-x-v1-set-fee-tiers-sepolia.js --network sepolia");
        process.exit(1);
    }
    ...
    console.log("==========================================");
    console.log("设置 MyXAuction 动态手续费档次");
    console.log("==========================================");
    ...
    try {
        ...
        try {
            ...
            if (version.toString() !== "1.0.0") {
                console.log("   ❌ 合约版本不是 V1，无法设置动态手续费");
                process.exit(1);
            }
        } catch (error) {
        ...
        const owner = await myXAuction.owner();
        ...
        const currentPlatformFee = await myXAuction.platformFee();
        const useDynamicFee = await myXAuction.useDynamicFee();
        const feeTierCount = await myXAuction.getFeeTierCount();
        ...
        if (feeTierCount > 0) {
            ...
            console.log("   ==========================================");
            const allTiers = await myXAuction.getAllFeeTiers();
            const baseFee = await myXAuction.baseFeeRate();
        ...
        try {
            ...
            const tx = await myXAuction.setFeeTiers(thresholds, feeRates);
            ...
            const newFeeTierCount = await myXAuction.getFeeTierCount();
            const newUseDynamicFee = await myXAuction.useDynamicFee();
            ...
            console.log("   ==========================================");
            const newAllTiers = await myXAuction.getAllFeeTiers();
            const newBaseFee = await myXAuction.baseFeeRate();
            ...
            for (const testCase of testCases) {
                ...
                const feeRate = await myXAuction.calculateDynamicFeeRate(usdValue);
                const feePercent = Number(feeRate) / 100;
                const feeAmount = (testCase.usd * feePercent) / 100;
                const sellerAmount = testCase.usd - feeAmount;
                // 格式化手续费率：如果是整数显示为整数，否则显示一位小数
                const feePercentFormatted = feePercent % 1 === 0
                    ? feePercent.toString() + "%"
                    : feePercent.toFixed(1) + "%";
                const match = feePercentFormatted === testCase.expected ? "✅" : "⚠️";

                console.log(`   ${testCase.desc}:`);
                console.log(`     手续费率: ${feePercentFormatted} ${match} (预期: ${testCase.expected})`);
        ...
        try {
            // 先禁用
            console.log("   禁用动态手续费...");
            const disableTx = await myXAuction.setDynamicFeeEnabled(false);
            await disableTx.wait();
            const disabled = await myXAuction.useDynamicFee();
            console.log(`   ✅ 动态手续费已禁用: ${disabled ? "否" : "是"}`);
            console.log();

            // 再启用
            console.log("   启用动态手续费...");
            const enableTx = await myXAuction.setDynamicFeeEnabled(true);
            await enableTx.wait();
            const enabled = await myXAuction.useDynamicFee();
