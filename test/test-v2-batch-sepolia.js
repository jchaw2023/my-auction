 ...
 * 
 * 指定起始索引和数量：
 * START=0 COUNT=5 npx hardhat run test/test-v2-batch-sepolia.js --network sepolia
 */
async function main() {
  // 检查网络
  if (network.name !== "sepolia") {
    console.error("❌ 此脚本只能在 Sepolia 测试网上运行");
    console.log("请使用: npx hardhat run test/test-v2-batch-sepolia.js --network sepolia");
    process.exit(1);
  }
  ...
  // 获取参数
  const startRaw = process.env.START || 
    process.argv.find(arg => arg.toLowerCase().startsWith("start="))?.split("=")[1];
  const countRaw = process.env.COUNT || 
    process.argv.find(arg => arg.toLowerCase().startsWith("count="))?.split("=")[1];
  
  const startIndex = startRaw ? parseInt(startRaw.trim()) : 0;
  const count = countRaw ? parseInt(countRaw.trim()) : 5;
  ...
  try {
    ...
    try {
      ...
      if (version.toString() !== "2") {
        console.log("   ❌ 合约版本不是 V2，无法测试批量获取功能");
        process.exit(1);
      }
    } catch (error) {
    ...
    // 3. 验证参数
    if (startIndex < 0) {
      console.log("❌ 起始索引不能为负数");
      process.exit(1);
    }

    if (Number(startIndex) >= Number(totalCount)) {
      console.log(`❌ 起始索引 ${startIndex} 超出范围 (总拍卖数: ${totalCount})`);
      process.exit(1);
    }
